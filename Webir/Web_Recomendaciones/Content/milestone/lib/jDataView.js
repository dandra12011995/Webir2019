...
var jDataView = function (buffer, byteOffset, byteLength, littleEndian) {
	...
	if (!(compatibility.NodeBuffer && buffer instanceof Buffer) &&
		...
		throw new TypeError('jDataView Type error' + (typeof buffer));
	}
	...
	if (!this._isDataView) {
		...
		if (typeof byteOffset !== 'number') {
			throw new TypeError('jDataView Type error');
		}
		...
		if (typeof byteLength < 0) {
			throw new Error('INDEX_SIZE_ERR: DOM Exception 2');
		}
	}
	...
	if (byteOffset + byteLength > bufferLength) {
		throw new Error("INDEX_SIZE_ERR: DOM Exception 3");
	}
...
jDataView.prototype = {
	...
	getString: function (length, byteOffset) {
		...
		if (length < 0 || byteOffset + length > this.byteLength) {
			throw new Error('INDEX_SIZE_ERR: DOM Exception 4');
		}
	...
	getArray: function (length, byteOffset) {
		var value;
		...
		if (length < 0 || byteOffset + length > this.byteLength) {
			console.log('length: ' + length + ' byteOffset: ' + byteOffset + ' this length: ' + this.byteLength);
			throw new Error('INDEX_SIZE_ERR: DOM Exception 5');
		}

		if (this._isArrayBuffer) {
			var res = new Uint8Array(this.buffer, byteOffset, length);
			this._offset = byteOffset + length;
			return res;
		...
		value = [];
		for (var i = 0; i < length; ++i) {
			value.push(this.getUint8(byteOffset + i));
		}
	...
	getUUID: function (byteOffset) {
		if (byteOffset === undefined) {
			byteOffset = this._offset;
		}
		
		var arr = this.getArray(16, byteOffset);
		var res = '';
		var map = ['a', 'b', 'c', 'd', 'e', 'f'];
	...
	seek: function (byteOffset) {
		...
		if (byteOffset < 0 || byteOffset > this.byteLength) {
			throw new Error('INDEX_SIZE_ERR: DOM Exception 6');
		}
	...
	/*
	 // Not all implementations of DataView and etc support 64 bit reading! 
	_getUint64: function (byteOffset, littleEndian) {
		var b7 = this._getUint8(this._endianness(byteOffset, 0, 8, littleEndian)),
			b6 = this._getUint8(this._endianness(byteOffset, 1, 8, littleEndian)),
			b5 = this._getUint8(this._endianness(byteOffset, 2, 8, littleEndian)),
			b4 = this._getUint8(this._endianness(byteOffset, 3, 8, littleEndian));
			b3 = this._getUint8(this._endianness(byteOffset, 4, 8, littleEndian)),
			b2 = this._getUint8(this._endianness(byteOffset, 5, 8, littleEndian)),
			b1 = this._getUint8(this._endianness(byteOffset, 6, 8, littleEndian)),
			b0 = this._getUint8(this._endianness(byteOffset, 7, 8, littleEndian));

		return (b7 * Math.pow(2, 56)) + (b6 * Math.pow(2, 48)) + (b5 * Math.pow(2, 40)) + (b4 * Math.pow(2, 32)) + 
			...
			function (byteOffset, littleEndian) {
				...
				// NodeJS Buffer
					...
					if (byteOffset + size > this.byteLength) {
						throw new Error('INDEX_SIZE_ERR: DOM Exception 7');
					}
